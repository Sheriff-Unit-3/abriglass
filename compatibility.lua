...
-- https://forum.luanti.org/viewtopic.php?p=396571&sid=17cb7c20fb158e9b994ea08fee4509cf#p396571 is true

local S = core.get_translator(core.get_current_modname())

local stainedoldnodes = {
	"abriglass:stained_glass_black",
	"abriglass:stained_glass_blue",
	"abriglass:stained_glass_cyan",
	"abriglass:stained_glass_green",
	"abriglass:stained_glass_magenta",
	"abriglass:stained_glass_orange",
	"abriglass:stained_glass_purple",
	"abriglass:stained_glass_red",
	"abriglass:stained_glass_yellow",
...
core.register_lbm({
	...
	action = function(pos, node)
		core.set_node(pos, { name = "abriglass:stained_glass_hardware", param2 = stainedoldnodes_to_new[node.name] })
	end,
})
...
for _, nodename in pairs(stainedoldnodes) do
	...
	core.register_craftitem(nodename, {
		description = S("@1 Glass", splitname[#splitname]),
		wield_image = "[combine:1x1", -- no way to fake this, so nothing for now
		inventory_image = core.inventorycube(textureside),
		groups = { not_in_creative_inventory = 1 },
		on_place = function(itemstack, placer, pointed_thing)
			local newstack = ItemStack(
				core.itemstring_with_palette("abriglass:stained_glass_hardware", stainedoldnodes_to_new[nodename])
			)
			...
			return core.item_place(newstack, placer, pointed_thing)
		end,
	})
end
...
local lightoldnodes = {
	"abriglass:glass_light_green",
	"abriglass:glass_light_blue",
	"abriglass:glass_light_yellow",
	"abriglass:glass_light_red",
}
...
core.register_lbm({
	...
	run_at_every_load = false,
	action = function(pos, node)
		core.set_node(pos, { name = "abriglass:glass_light_hardware", param2 = lightstainedoldnodes_to_new[node.name] })
	end,
})
...
for _, nodename in pairs(lightoldnodes) do
	...
	core.register_craftitem(nodename, {
		description = S("@1 Glass Light", splitname[#splitname]),
		wield_image = "[combine:1x1", -- no way to fake this, so nothing for now
		inventory_image = core.inventorycube(textureside),
		groups = { not_in_creative_inventory = 1 },
		on_place = function(itemstack, placer, pointed_thing)
			local newstack = ItemStack(
				core.itemstring_with_palette("abriglass:glass_light_hardware", lightstainedoldnodes_to_new[nodename])
			)

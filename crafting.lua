...
for k, v in pairs(dye_list) do
	local out_item = ItemStack(core.itemstring_with_palette("abriglass:stained_glass_hardware", k - 1))
	...
	out_item = ItemStack(core.itemstring_with_palette("abriglass:glass_light_hardware", k - 1))
...
local function craft_encode(input)
	...
	for _, v in ipairs(input) do --care about order
		...
		for _, j in ipairs(v) do --care about order
			...
			row = row
				.. item:get_name()
				.. (item:get_meta():get("description") or "")
				.. (item:get_meta():get("palette_index") or "")
		end
		output = output .. "," .. row
	end
	return output
...
local function hcc_1_func(itemstack, player, old_craft_grid, craft_inv)
	...
	if
		itemstack:get_name() ~= "abriglass:stainedglass_pattern05"
		and itemstack:get_name() ~= "abriglass:stainedglass_tiles_pale"
	then
		return
	end
